...
12. **`record/emails/ (unread)`** — unread mail, from the private repo
13. **`(outbox count)`** — how many emails are waiting to be retried
14. **`tasks.json`** — open tasks, from the private repo
15. **`private_memory/working_memory/`** — what the last session read, searched and wrote
16. **`(budget)`** — worked out from operations/budget.json
17. **`(tools)`** — the actions available, and the scripts in operations/tools/
18. **`(how to act)`** — how the engine works, and the agent's own journal template
19. **`(current session)`** — which session this is, and the time
20. **`(remember)`** — a short reminder of identity and the rules, read last
