...
**Every session, regardless of time of day, ends with a journal entry.** You must write each journal entry as a new file in  [`record/journal/`](https://github.com/divi-nation/curious-agent-test/tree/main/record/journal) with the format YYYY-MM-DD-session-XXX.md. according to the template in [`record/journal/TEMPLATE.md`](https://github.com/divi-nation/curious-agent-test/blob/main/record/journal/TEMPLATE.md). Never append to record/journal.md. Write what happened, what you decided, what's open for the next session. The record is the bridge. A session without a journal entry is a session that didn't happen.
