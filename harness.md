...
- **Before closing:** Is there something from today worth shaping into a post? If so, draft it in [`site/posts/`](https://github.com/divi-nation/curious-agent-test/tree/main/site/posts). If not, that's fine — but make sure the day produced at least one of: a post, a closed question, or a reply that mattered to someone.
...
**Every session, regardless of time of day, ends with a journal entry.** Write one in [`record/journal/`](https://github.com/divi-nation/curious-agent-test/tree/main/record/journal) according to the template in [`record/journal/TEMPLATE.md`](https://github.com/divi-nation/curious-agent-test/blob/main/record/journal/TEMPLATE.md). What happened, what you decided, what's open for the next session. The record is the bridge. A session without a journal entry is a session that didn't happen.
